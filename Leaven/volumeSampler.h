...
public:
   ...
   * @param cellSize cell size in which each sampling particle lies. usually
   * particle diameter
   * @param maxSamples maximum number of sampling particles. -1 for dense
   * filling
   * @param invert samples the volume between the outside of the mesh and the
   * bounding box of the mesh
   * @param sdfResolution resolution of the SDF
   * @return sampled particles
   */
  static std::vector<Eigen::Matrix<scalar, 3, 1>> sampleMeshDense(
      const Eigen::Matrix<scalar, 3, Eigen::Dynamic> &vertices,
      const Eigen::Matrix<unsigned int, 3, Eigen::Dynamic> &indices,
      const scalar &partRadius, const scalar &cellSize,
      const int &maxSamples = -1, const bool &invert = false,
  ...
  /**
   * Fills a given mesh with random sampled points inside the volume without
   * particles intersecting each other
   * @param vertices vertices mesh vertices
   * @param indices mesh face indices
   * @param partRadius sample particle radius
   * @param numTrials # of trial iterations used to find samples in each valid
   * cell
   ...
   * @return sampled particles
   */
  static std::vector<Eigen::Matrix<scalar, 3, 1>> sampleMeshRandom(
      const Eigen::Matrix<scalar, 3, Eigen::Dynamic> &vertices,
      const Eigen::Matrix<unsigned int, 3, Eigen::Dynamic> &indices,
      const scalar &partRadius, const unsigned int &numTrials = 10,
      const scalar &initialPointsDensity = 40, const bool &invert = false,
      const std::array<unsigned int, 3> &sdfResolution = {
          static_cast<unsigned int>(20), static_cast<unsigned int>(20),
          static_cast<unsigned int>(20)});
...
private:
  ...
  generateSDF(const Eigen::Matrix<scalar, 3, Eigen::Dynamic> &vertices,
              const Eigen::Matrix<unsigned int, 3, Eigen::Dynamic> &indices,
  ...
  static void parallelUniformVolumeSampling(
      ...
      std::vector<std::vector<Eigen::Vector3i>> &phaseGroups);
};
