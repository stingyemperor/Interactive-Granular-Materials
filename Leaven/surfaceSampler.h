 ...
 * J. Bowers, et. al. Parallel poisson disk sampling with spectrum analysis on
 * surfaces. ACM Trans. Graph., 29(6), December 2010
 */
class SurfaceSampler {
protected:
#ifdef USE_DOUBLE
...
public:
   ...
   */
  static std::vector<Eigen::Matrix<scalar, 3, 1>>
  sampleMesh(const Eigen::Matrix<scalar, 3, Eigen::Dynamic> &vertices,
...
protected:
  ...
  static void calculateTriangleAreas(
      ...
      const Eigen::Matrix<scalar, 3, Eigen::Dynamic> &vertices,
      const Eigen::Matrix<unsigned int, 3, Eigen::Dynamic> &indices);
  static void generateInitialSetP(
