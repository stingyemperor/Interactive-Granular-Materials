...
std::vector<Eigen::Matrix<scalar, 3, 1>> SurfaceSampler::sampleMesh(
    ...
    const scalar &initialPointsDensity, const unsigned int &distanceNorm) {
  ...
  const auto numInitialPoints =
      static_cast<uint>(initialPointsDensity * (totalArea / circleArea));
...
void SurfaceSampler::computeFaceNormals(std::vector<Vector3> &faceNormals,
...
void SurfaceSampler::calculateTriangleAreas(
    std::vector<scalar> &areas, scalar &maxArea, scalar &totalArea,
    const Eigen::Matrix<scalar, 3, Eigen::Dynamic> &vertices,
    const Eigen::Matrix<unsigned int, 3, Eigen::Dynamic> &indices) {
...
void SurfaceSampler::generateInitialSetP(
    std::vector<PossiblePoint> &possiblePoints,
    const std::vector<scalar> &areas, const scalar &maxArea,
    const Matrix3X &vertices, const Indices &indices) {
#pragma omp parallel default(shared)
  {
...
unsigned int SurfaceSampler::generateRandomTriangleIndex(
    const std::vector<scalar> &areas, const scalar &maxArea, std::mt19937 &mt,
    std::uniform_real_distribution<scalar> &uniformDist) {
  bool notValid = true;
  uint index;
  while (notValid) {
    index =
        static_cast<uint>(static_cast<scalar>(areas.size()) * uniformDist(mt));
...
void SurfaceSampler::parallelUniformSurfaceSampling(
    ...
    const unsigned int &numTrials, const scalar &minRadius,
  ...
  std::unordered_map<CellPos, HashEntry, HashFunc> hMap(2 *
                                                        possiblePoints.size());
  samples.clear();
