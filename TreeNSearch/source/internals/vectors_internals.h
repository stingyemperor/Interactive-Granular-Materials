...
#include "utils/simde/simde/simde-common.h"
#include "utils/simde/simde/x86/avx.h"
#endif
...
*/
template <typename T, std::size_t N = 32> class AlignmentAllocator {
public:
 ...
 */
template <typename T, size_t CHUNKSIZE = 1000> class chunked_vector {
private:
  /* Fields */
...
public:
  ...
  T *get_cursor_with_space_to_write(const size_t n) {
    if (n > CHUNKSIZE) {
      std::cout
          << "chunked_vector compare: Cannot allow_to_append n > CHUNKSIZE ("
    ...
    const size_t space_left =
        CHUNKSIZE -
        (size_t)std::distance(this->chunks[this->current][0],
                              this->chunks[this->current][1]) -
        1;
    if (n > space_left) {
      this->current++;
 ...
 */
template <typename T> class uvector {
public:
  T *data = nullptr;
  T *end = nullptr;
  ...
  uvector(const uvector &other) {
    if (other.data != nullptr) {
      ...
      memcpy(this->data, other.data,
             sizeof(T) * std::distance(other.data, other.end));
    } else {
      this->data = nullptr;
      this->end = nullptr;
      this->cursor = nullptr;
  ...
  void grow_while_keeping_data(const int n) {
    ...
    if (n > this->capacity()) {
      ...
      T *data_ = new T[n];
      memcpy(data_, this->data,
             sizeof(T) * this->capacity()); // Cursor is handled by the user
      delete[] this->data;
      this->data = data_;
  ...
  template <typename INDEX_TYPE> T &operator[](INDEX_TYPE idx) {
    return this->data[idx];
  }

  template <typename INDEX_TYPE> T operator[](INDEX_TYPE idx) const {
    return this->data[idx];
  }
};
} // namespace internals
