...
public:
  // Fields
  std::array<T, 3> bottom = {std::numeric_limits<T>::max(),
                             std::numeric_limits<T>::max(),
                             std::numeric_limits<T>::max()};
  std::array<T, 3> top = {std::numeric_limits<T>::lowest(),
  ...
  inline AABB<T> get_extended(const T extension) const {
    AABB<T> extended;
    extended.bottom = {this->bottom[0] - extension, this->bottom[1] - extension,
                       this->bottom[2] - extension};
    extended.top = {this->top[0] + extension, this->top[1] + extension,
                    this->top[2] + extension};
    return extended;
  };
  ...
  inline bool intersects(const AABB<T> &other) const {
    for (size_t dim = 0; dim < 3; dim++) {
      if ((this->bottom[dim] > this->top[dim]) ||
          (other.top[dim] < other.bottom[dim])) {
        return false;
      }
    }
    return true;
 ...
 * the octree.
 */
template <typename NODE_BUFFER, size_t N> class RecursiveBuffer {
public:
  // Fiedls
  NODE_BUFFER buffer;
  ...
  inline ~RecursiveBuffer() { this->delete_children(); };
...
/**
 * Container used to allocate the temporal data required to solve the brute
 * force of the neighborhood search.
 */
