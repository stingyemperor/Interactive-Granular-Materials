...
#if defined(_MSC_VER)
    std::copy(other.m_facesEdges[i].data(),
              other.m_facesEdges[i].data() + m_verticesPerFace,
              stdext::unchecked_array_iterator<unsigned int *>(
                  m_facesEdges[i].data()));
#else
    std::copy(other.m_facesEdges[i].data(),
              other.m_facesEdges[i].data() + m_verticesPerFace,
...
void IndexedFaceMesh::initMesh(const unsigned int nPoints,
