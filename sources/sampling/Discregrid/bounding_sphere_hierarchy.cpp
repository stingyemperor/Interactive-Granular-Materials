...
void TriangleMeshBBH::computeHull(unsigned int b, unsigned int n,
                                  AlignedBox3d &hull) const {
  for (auto i = 0u; i < n; ++i) {
    auto const &f = m_faces[m_lst[b + i]];
    for (auto v : f) {
...
void PointCloudBSH::computeHull(unsigned int b, unsigned int n,
                                BoundingSphere &hull) const {
