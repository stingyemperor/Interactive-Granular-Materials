...
MeshDistance::MeshDistance(TriangleMesh const &mesh, bool precompute_normals)
  ...
  m_cache.resize(
      max_threads,
      FunctionValueCache([&](Vector3d const &xi) { return signedDistance(xi); },
                         10000u));
  m_ucache.resize(
      max_threads,
      FunctionValueCache([&](Vector3d const &xi) { return distance(xi); },
                         10000u));
  ...
  if (m_precomputed_normals) {
    ...
    std::transform(
        ...
        [&](std::array<unsigned int, 3> const &face) {
          ...
          auto alpha = Vector3d{std::acos(e1.dot(-e3)), std::acos(e2.dot(-e1)),
                                std::acos(e3.dot(-e2))};
...
double MeshDistance::distance(Vector3d const &x, Vector3d *nearest_point,
                              unsigned int *nearest_face,
                              NearestEntity *ne) const {
...
bool MeshDistance::predicate(unsigned int node_index,
                             TriangleMeshBSH const &bsh, Vector3d const &x,
